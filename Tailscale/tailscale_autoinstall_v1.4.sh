...
apt-get install -yq curl sudo ca-certificates gnupg lsb-release >/dev/null 2>&1
success "依赖检查完成。"
...
if command -v tailscale >/dev/null 2>&1; then
  success "检测到 Tailscale 已安装，跳过安装步骤。"
else
  ...
  retry apt-get update -yq
  retry apt-get install -yq tailscale
